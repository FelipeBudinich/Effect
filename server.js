...
app.get('/', async (req, res) => {
    try {
        if (process.env.BAKED === 'false'){
            await runBaker();
        }
        res.sendFile(path.join(root, 'release.html'));
    } catch (error) {
...
const port = process.env.PORT || 3000;  // Heroku provides the PORT variable
app.listen(port, () => {
  console.log(`Server is running on http://localhost:${port}`);
  if (process.env.BAKED === 'true') {
    console.log('The release is static.');
  }
});
